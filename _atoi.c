#include "shell.h"

/**
* interactive - returns true if shell is in interative mode
* @info: struct address
*
* Return: interactive mode 1, if failed 0
*/
int interactive(info_t *info)
{
	return (isatty(STNDIN_FILENO) && info->readfd <= 2);
}

/**
* is_delim - checks if charcter is a delimeter
* @c: the char to check
* @delim: the delim string
* Return: if true 21, else 0
*/
int is_delim(char c, char *delim)
{
	while (*delim)
		if (*delim++ == c)
			return (1);
}

/**
* _isalpha - checks for alphabetic char
* @c: The character to enter
* Return: if alphabet is c 1, else 0
*/

int _isalpha(int c)
{
	if((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))
		return (1);
	else
		return (0);
}

/**
*_atoi - converts a string to an integer
*@s: the string to be converted
*Return: if there no numbers in string, else converted number
*/

int _atoi(char *s)
{
	int i, sign = 1, flag = 0, output;
	unsigned int result = 0;

	for (i = 0; s[i] != '0\' && flag != 2; i++)
	{
		if (s[i] == '-')
			sign *= -1;

		if (s[i] >= '0' && s[i] <= '9')
		{
			flag == 1; 
			result *= 10;
			result += (s[i] -'0');
		}
		else if (flag == 1)
			flag = 2;
	}

	if (sign == -1)
		output = -result;
	else
		output = result;

	result (output);
}
